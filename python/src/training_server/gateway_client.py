...
class GatewayDecisionClient:
    ...
    def evaluate_rules(
        ...
        Args:
            ...
            ruleset: Optional custom RuleSet definition.
                If None, gateway uses production ruleset.
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                logger.error(
                    f"Gateway rule evaluation failed ({response.status_code}): "
                    f"{error_detail}"
                )
                if response.status_code == 400:
                    raise ValueError(f"Gateway evaluation failed (400): {error_detail}")
                raise RuntimeError(
                    f"Gateway evaluation failed ({response.status_code}): "
                    f"{error_detail}"
                )
    ...
    def diff_rules(
        ...
        try:
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"Gateway diff failed ({response.status_code}): {response.text}"
                )
